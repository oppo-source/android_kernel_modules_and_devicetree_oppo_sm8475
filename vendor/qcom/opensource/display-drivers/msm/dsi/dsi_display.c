...
#ifdef OPLUS_FEATURE_DISPLAY
	/* Initialize the panel->display_type */
	display->panel->display_type = display->display_type;
#endif /* OPLUS_FEATURE_DISPLAY */
...
static struct device_node *_iris_dsi_display_get_panel_node(struct platform_device *pdev, int panel_index)
{
	...
	if (panel_index < IRIS_PANEL_NOT_SUPPORT || panel_index > IRIS_PANEL_SUPPORT_II)
		return NULL;
