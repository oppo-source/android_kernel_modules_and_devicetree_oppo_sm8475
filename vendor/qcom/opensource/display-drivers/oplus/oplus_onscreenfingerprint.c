...
int oplus_ofp_init(void *dsi_panel)
{
	...
	if (oplus_ofp_is_supported()) {
		...
		p_oplus_ofp_params->need_to_block_frame_after_aod_on = utils->read_bool(utils->data, "oplus,ofp-need-to-block-frame-after-aod-on");
		OFP_INFO("need_to_wait_data_before_aod_on:%d need_to_block_frame_after_aod_on:%d\n",
			p_oplus_ofp_params->need_to_wait_data_before_aod_on, p_oplus_ofp_params->need_to_block_frame_after_aod_on);
...
bool oplus_get_aod_wait_status(void)
{
	struct oplus_ofp_params *p_oplus_ofp_params = oplus_ofp_get_params(oplus_ofp_display_id);
	if(p_oplus_ofp_params->need_to_block_frame_after_aod_on && p_oplus_ofp_params->block_frame_after_aod_on) {
		p_oplus_ofp_params->block_frame_after_aod_on = false;
		return true;
	}
	return false;
}
...
int oplus_ofp_aod_display_on_set(void *sde_encoder_phys)
{
	...
	if (oplus_ofp_get_aod_state() && p_oplus_ofp_params->wait_data_before_aod_on) {
		...
		p_oplus_ofp_params->block_frame_after_aod_on = true;
