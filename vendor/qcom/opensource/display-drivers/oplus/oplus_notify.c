		...
		bool early_trigger)
{
	...
	if (!strcmp(panel->display_type, "secondary")) {
		panel_type = PANEL_EVENT_NOTIFICATION_SECONDARY;
	} else {
		panel_type = PANEL_EVENT_NOTIFICATION_PRIMARY;
	}

	LCD_DEBUG_COMMON("Trigger notifier, display:%s, panel:%d, type:%d, data:%d, early_trigger:%d\n",
			panel->display_type, panel_type, notif_type, data, early_trigger);
