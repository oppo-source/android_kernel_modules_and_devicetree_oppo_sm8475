		...
		if (timing.h_skew == SDC_ADFR || timing.h_skew == SDC_MFR) {
			if (timing.refresh_rate == 90) {
				if (!strcmp(display->panel->oplus_priv.vendor_name, "TM_NT37705")) {
					if (temp_refresh_rate > 38 && temp_refresh_rate <= 55) {
					...
					} else if (temp_refresh_rate > 25 && temp_refresh_rate <= 38) {
						mid_refresh_rate_count++;
						high_refresh_rate_count = 0;
						if (mid_refresh_rate_count == 1) {
				...
				if (temp_refresh_rate > 56 && temp_refresh_rate <= 90) {
					mid_refresh_rate_count++;
					high_refresh_rate_count = 0;
					if (mid_refresh_rate_count == 1) {
				...
				} else if (temp_refresh_rate > 40 && temp_refresh_rate <= 56) {
                                        mid_refresh_rate_count++;
                                        high_refresh_rate_count = 0;
                                        if (mid_refresh_rate_count == 1) {
                                                oplus_adfr_dynamic_te.refresh_rate = 48;
                                                mid_refresh_rate_count--;
                                        }
				} else {
                                	mid_refresh_rate_count = 0;
				...
				if (temp_refresh_rate > 55) {
					...
					if (!strcmp(display->panel->oplus_priv.vendor_name, "TM_NT37705")) {
						if (temp_refresh_rate > 35 && temp_refresh_rate <= 45) {
							mid_refresh_rate_count++;
							high_refresh_rate_count = 0;
							if (mid_refresh_rate_count == 1) {
								oplus_adfr_dynamic_te.refresh_rate = 40;
								mid_refresh_rate_count--;
							}
						} else if (temp_refresh_rate > 25 && temp_refresh_rate <= 35) {
							mid_refresh_rate_count++;
							high_refresh_rate_count = 0;
							if (mid_refresh_rate_count == 1) {
								oplus_adfr_dynamic_te.refresh_rate = 30;
		...
		int ctrl_idx, uint32_t min_fps)
{
	...
	if(i >= 0 && i < priv_info->qsync_min_fps_sets_size) {
		...
		if (!strcmp(panel->oplus_priv.vendor_name, "TM_NT37705")) {
			char value[] = {0x00, 0x00, 0x00, 0x00, 0x00};
			mipi_dsi_dcs_write(&panel->mipi_device, 0x35, value, sizeof(value));
		}
	} else {
...
int dsi_display_auto_mode_update(void *dsi_display)
{
	...
	if (oplus_adfr_auto_min_fps_updated) {
		...
		if (!strcmp(display->panel->oplus_priv.vendor_name, "TM_NT37705")) {
			char reg[1] = {0x00};
			dsi_display_read_panel_reg(display, 0x6D, reg, 1);
		}
