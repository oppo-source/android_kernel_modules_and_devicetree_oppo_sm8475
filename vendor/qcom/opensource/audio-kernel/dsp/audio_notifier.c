...
// merge qcom debug patch, to track fastrpc issue
		pr_err("%s: audio notifier registered to PDR service done handle = 0x%pk\n", __func__, handle);
...
// merge qcom debug patch, to track fastrpc issue
	pr_info("%s: service %s is in use\n",
		__func__, service_data[service][domain].name);
#else
	pr_err("%s: service %s is in use , state = %d\n",
		__func__, service_data[service][domain].name, service_data[service][domain].state);
#endif /*OPLUS_ARCH_EXTENDS*/
...
#ifdef OPLUS_ARCH_EXTENDS
// merge qcom debug patch, to track fastrpc issue
	pr_err("%s service = %d domain = %d state = %d client_service = %d\n", __func__, service, domain, service_data[service][domain].state, client_data->service);
#endif /*OPLUS_ARCH_EXTENDS*/
...
#ifdef OPLUS_ARCH_EXTENDS
// merge qcom debug patch, to track fastrpc issue
	pr_err("%s: status = 0x%x, service_name = %s \n", __func__, status, service_name);
#endif /*OPLUS_ARCH_EXTENDS*/
...
#ifdef OPLUS_ARCH_EXTENDS
// merge qcom debug patch, to track fastrpc issue
	pr_err("%s: audio_notifier_reg_client for client %s done\n", __func__, client_data->client_name);
#endif /*OPLUS_ARCH_EXTENDS*/
