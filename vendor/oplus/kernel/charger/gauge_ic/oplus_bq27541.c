...
static atomic_t gauge_i2c_status;
	...
	*returnData = oplus_i2c_smbus_read_word_data(chip->client, cmd);
	...
	if (*returnData < 0) {
		...
		if (atomic_read(&chip->suspended) == 0) {
			...
			atomic_set(&gauge_i2c_status, 1);
		}
		return 1;
	} else {
		/*clear the i2c_status*/
		if (atomic_read(&gauge_i2c_status))
			atomic_set(&gauge_i2c_status, 0);
		return 0;
	}
}
...
static bool zy0603_fg_condition_check(void)
{
	...
	if (atomic_read(&gauge_ic->suspended) == 1 || atomic_read(&gauge_i2c_status)) {
...
static int bq27541_get_sub_battery_cc(void)
{
	...
	if (atomic_read(&sub_gauge_ic->suspended) == 1 || atomic_read(&gauge_i2c_status)) {
...
static int bq28z610_get_device_chemistry(struct chip_bq27541 *chip)
{
	...
	if (atomic_read(&chip->suspended) == 1 || atomic_read(&gauge_i2c_status)) {
		return 0;
	}
