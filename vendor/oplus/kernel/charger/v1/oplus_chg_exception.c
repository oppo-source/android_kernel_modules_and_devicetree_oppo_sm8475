...
static unsigned int oplus_chg_olc_get_excep_reason(int type_reason, int flag_reason, int *diff)
{
	unsigned int excep_value = 0;
...
int chg_exception_report(void *chg_exception_data, int type_reason, int flag_reason, void *summary, unsigned int summary_size)
{
	...
	int type_diff = 0;
	int curr_time = 0;
	unsigned int excep_chgye = 0;
	static int upload_count = 0;
	static int pre_upload_time = 0;
	static unsigned int pre_excep_chgye = 0;
	...
	curr_time = olc_track_get_local_time_s();
	...
	if (excep_chgye == pre_excep_chgye) {
	...
	pre_excep_chgye = excep_chgye;
	pre_upload_time = curr_time;

	if ((should_upload == false) || (upload_count > MAX_UPLOAD_COUNT)) {
		chg_err("%s: not allow upload olc type\n", __func__);
		return ret;
	}
...
EXPORT_SYMBOL(chg_exception_report);
