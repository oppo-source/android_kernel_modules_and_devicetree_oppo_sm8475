...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_PIPELINE)
	/*
	 * pipeline task ux_state is SA_TYPE_SWIFT, 8ms is insufficient for some pipeline tasks,
	 * such as UnityMain UnityGfx, increase exec_limit to 16ms.
	 */
	if (oplus_is_pipeline_task(p)) {
		exec_limit *= 4;
		return exec_limit;
	}
#endif
